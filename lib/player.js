const Player = class extends EventEmitter {
  ...
  playNote (note) {
    ...
    if (note.type === Note.types.WAVE) {
      const imag = new Float32Array(note.wave)
    ...
    const envelopeData = note.envelope

    if (envelopeData.length > 0) {
      let i
      for (i = 0; i < envelopeData.length; i += 2) {
        const time = envelopeData[i]
        const value = Math.min(Math.max(envelopeData[i + 1], .0001), 1)
        envelope.gain.exponentialRampToValueAtTime(value * note.volume, currentTime + time * note.duration)
      }
    } else {
